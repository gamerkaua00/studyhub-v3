// StudyHub v3 — scheduler.js — D-1 às 08:00 E 17:00
...
const D1_TIMES = ["08:00", "17:00"]; // avisa D-1 às 8h E às 17h
...
const runScheduler = async () => {
  try {
    ...
    for (const c of dueContents) {
      ...
      } catch (err) { console.error(`[Scheduler] ❌ Notif ${c.title}:`, err.message); }
    }

    // 2. Aviso D-1 — às 08:00 E às 17:00
    ...
    // CATCH-UP: prova hoje, D-1 não enviado, horário ainda não passou
    const missedD1 = await Content.find({
      date: today,
      type: { $in: ["Prova", "Avaliação"] },
      "notifications.sentDayBefore": false,
    });
    ...
    // 5. Mensagens programadas (janela de 5 min)
    ...
    for (const msg of dueMessages) {
      ...
      const inWin   = isToday && nowMin >= msgMin && nowMin <= msgMin + 5;
      if (inWin || isPast) {
        try {
          const result = await sendScheduledMessage(msg);
          await Message.findByIdAndUpdate(msg._id, { $set: { sent: true } });
          console.log(`[Scheduler] ${result ? "✅" : "⚠️"} Mensagem: "${msg.title || msg.content.substring(0, 30)}"`);
        } catch (err) { console.error(`[Scheduler] ❌ Mensagem:`, err.message); }
      }
    }
  } catch (err) { console.error("[Scheduler] ❌ Erro geral:", err.message); }
};
...
const sendWeeklySummary = async (today) => {
  try {
    ...
    await sendDiscordNotification("agenda", "@here 📆 **Resumo semanal:**", { title:"📆 Resumo Semanal", description:lines, color:0x9B59B6, timestamp:new Date().toISOString(), footer:{text:`StudyHub • ${contents.length} item(ns)`} });
