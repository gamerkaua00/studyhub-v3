// StudyHub v3.2.0 — services/discordNotifier.js
// Webhooks com fallback para bot token
...
const findChannelId = async (channelName) => {
  ...
  return new Promise((resolve) => {
    const opts = { hostname:"discord.com", path:`/api/v10/guilds/${guildId}/channels`, method:"GET", headers:{Authorization:`Bot ${token}`} };
    const req = https.request(opts, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          const found = channels.find((c) => c.name?.toLowerCase() === channelName.replace("#","").toLowerCase() && c.type === 0);
...
const sendViaBot = (channelId, body) => {
  const token   = process.env.DISCORD_BOT_TOKEN;
...
const sendViaWebhookUrl = (webhookUrl, body) => {
  const payload = JSON.stringify(body);
  return new Promise((resolve) => {
...
const sendDiscordNotification = async (channelName, content = "", embed = null) => {
  ...
  const body = { content: content || "", embeds: embed ? [embed] : [] };

  // 1. Tenta webhook cadastrado
  try {
    const Webhook = require("../models/Webhook");
    const hook = await Webhook.findOne({ channelName: name, active: true });
    if (hook) {
      const ok = await sendViaWebhookUrl(hook.url, body);
      if (ok) { console.log(`[Discord] ✅ Webhook: #${name}`); return true; }
      console.warn(`[Discord] ⚠️ Webhook falhou: #${name}, tentando bot...`);
    }
  } catch {}

  // 2. Fallback: bot token
  if (!process.env.DISCORD_BOT_TOKEN) { console.warn(`[Discord] Sem token: #${name}`); return null; }
  const channelId = await findChannelId(name);
  if (!channelId) { console.warn(`[Discord] Canal não encontrado: #${name}`); return null; }
  const result = await sendViaBot(channelId, body);
  if (result) console.log(`[Discord] ✅ Bot: #${name}`);
  return result;
};
...
const sendMainNotification = async (content) => {
  ...
  return sendDiscordNotification(content.discordChannel || "conteudos", `${ping}**${content.type}** de **${content.subject}**`, embed);
};

const sendThirtyMinNotification = async (content) => {
  const [y,m,d] = content.date.split("-");
  return sendDiscordNotification(content.discordChannel || "conteudos", "", {
    author: { name:"⏰ Lembrete — 30 minutos" },
    title: content.title,
    description: `**${content.type}** de **${content.subject}** começa em 30 minutos!`,
    color: 0xFEE75C,
    fields: [{ name:"🕐 Horário", value:content.time, inline:true }],
    timestamp: new Date().toISOString(),
...
const sendDayBeforeNotification = async (content) => {
  const [y,m,d] = content.date.split("-");
  return sendDiscordNotification(content.discordChannel || "avisos-provas", "@here ⚠️ **Prova amanhã!**", {
...
const sendExamPoll = async (content) => {
  const [y,m,d] = content.date.split("-");
  return sendDiscordNotification(content.discordChannel || "avisos-provas", "", {
    author: { name:"📊 Enquete de Estudos — D-3" },
    title: content.title,
    description: `A prova de **${content.subject}** acontece em **3 dias** (${d}/${m}/${y}).\n\nReact com ✅ se já está estudando ou ❌ se ainda não começou!`,
    color: 0xEB459E,
    fields: [{ name:"🕐 Horário da Prova", value:content.time, inline:true }],
    timestamp: new Date().toISOString(),
...
const sendScheduledMessage = async (message) => {
  const result = await sendDiscordNotification(message.discordChannel, "", {
...
const sendGalleryPhoto = async (photo) => {
  const ch = photo.subject.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"");
  const [y,m,d] = photo.date.split("-");
  const labels = { aula:"Aula",prova:"Prova",atividade:"Atividade",avaliacao:"Avaliação",apresentacao:"Apresentação",lista:"Lista",outro:"Outro" };
  return sendDiscordNotification(ch, "", {
    author: { name:`📷 ${labels[photo.photoType]||"Foto"} — ${photo.subject}` },
...
const sendAttendancePanel = async (attendance) => {
  ...
  if (!channelName) { console.warn(`[Discord] Canal atendimento não definido: ${attendance.subject}`); return null; }
  ...
  const scheduleFields = schedules.map((s, i) => {
    ...
    return { name: schedules.length > 1 ? `📅 Horário ${i+1}` : "📅 Horário", value: val, inline: true };
  });
  const result = await sendDiscordNotification(channelName, "", {
    author: { name:"📋 Horário de Atendimento" },
    title: attendance.subject,
    color: 0x1ABC9C,
    fields: [{ name:"👨‍🏫 Professor", value:attendance.teacher||"—", inline:false }, ...scheduleFields],
    timestamp: new Date().toISOString(),
    footer: { text:"StudyHub IFPR • Atualizado automaticamente" },
  });
  console.log(`[Discord] ${result ? "✅" : "⚠️"} Atendimento: #${channelName}`);
  return result;
};
...
const sendMonthlyAgenda = async (contents, month, year) => {
  ...
  for (let i = 0; i < chunks.length; i++) {
    await sendDiscordNotification("agenda", "", {
      ...
      color: 0x5865F2, fields: chunks[i],
      timestamp: new Date().toISOString(),
      footer: { text:`StudyHub IFPR • ${contents.length} item(ns)` },
    });
  }
  return true;
};
...
const notifyAdminNewRequest = async (username, role) => {
  return sendDiscordNotification("admin-bot", "", {
    ...
    title: username,
    description: `**${username}** solicitou acesso como **${role}**.\nAcesse o painel para aprovar ou rejeitar.`,
    color: 0xFEE75C,
    timestamp: new Date().toISOString(),
...
module.exports = {
  ...
  sendAttendancePanel, sendMonthlyAgenda, notifyAdminNewRequest, sendErrorLog, findChannelId,
};
